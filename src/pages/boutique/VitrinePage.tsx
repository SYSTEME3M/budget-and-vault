          ...
          <div className="bg-white w-full max-w-lg rounded-t-3xl max-h-[90vh] overflow-y-auto">
            <div className="p-4 space-y-4">
              <div className="flex justify-between items-start">
                <h2 className="font-black text-lg text-gray-800 flex-1 pr-2">{produitDetail.nom}</h2>
                ...
                <div className="flex gap-2 overflow-x-auto">
                  {produitDetail.photos.map((url, i) => (
                    <img key={i} src={url} alt=""
                      className="w-40 h-40 object-cover rounded-xl flex-shrink-0 border border-gray-100" />
                  ))}
                </div>
              )}
              ...
              <span className={`inline-block text-xs font-medium px-2 py-1 rounded-full ${
                produitDetail.type === "numerique"
                  ? "bg-blue-100 text-blue-700"
                  : "bg-gray-100 text-gray-600"
              }`}>
                {produitDetail.type === "numerique" ? "💻 Produit numérique" : "📦 Produit physique"}
              </span>
              ...
              <div className="flex items-center gap-3">
                ...
                {produitDetail.prix_promo && (
                  <span className="text-red-400 line-through text-lg font-bold">
                    {formatPrix(produitDetail.prix, boutique?.devise)}
                  </span>
                )}
                {produitDetail.prix_promo && (
                  <span className="bg-red-500 text-white text-xs font-black px-2 py-0.5 rounded-full">
                    -{calcPct(produitDetail.prix, produitDetail.prix_promo)}%
                  </span>
                )}
              </div>
              ...
              {produitDetail.description && (
                <p className="text-gray-500 text-sm">{produitDetail.description}</p>
              )}

              {/* Stock */}
              {produitDetail.type === "physique" && !produitDetail.stock_illimite && (
                <p className="text-xs text-gray-400">
                  Stock disponible : <span className="font-bold text-gray-700">{produitDetail.stock}</span>
                </p>
              )}

              {/* Variations — physique seulement */}
              {produitDetail.type === "physique" && produitDetail.variations?.map(v => (
                <div key={v.nom}>
                  <p className="text-sm font-semibold mb-2 text-gray-700">{v.nom}</p>
              ...
              {/* Quantité — physique seulement */}
              {produitDetail.type === "physique" && (
                <div>
                  <p className="text-sm font-semibold mb-2 text-gray-700">Quantité</p>
                  <div className="flex items-center gap-3">
                    <button onClick={() => setQte(q => Math.max(1, q - 1))}
                      className="w-10 h-10 rounded-xl border-2 border-gray-200 flex items-center justify-center text-gray-600 hover:border-pink-300">
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="text-xl font-black w-8 text-center text-gray-800">{qte}</span>
                    <button onClick={() => setQte(q => Math.min(produitDetail.stock_illimite ? 999 : produitDetail.stock, q + 1))}
                      className="w-10 h-10 rounded-xl border-2 border-gray-200 flex items-center justify-center text-gray-600 hover:border-pink-300">
              ...
              <div>
                <p className="text-sm font-semibold mb-2 text-gray-700">Moyens de paiement</p>
                <div className="flex gap-2 flex-wrap">
                  {produitDetail.paiement_reception && (
                    <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-lg font-medium">
                      ✅ À la réception
                    </span>
                  )}
                  {produitDetail.paiement_lien && (
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-lg font-medium">
                      🔗 Lien paiement
                    </span>
                  )}
                  {produitDetail.moyens_paiement?.map((mp, i) => (
                    <span key={i} className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded-lg font-medium">
                      📱 {mp.reseau}
                    </span>
                  ))}
                </div>
              ...
              <button
                onClick={() => addToCart(produitDetail, produitDetail.type === "numerique" ? 1 : qte, variationsChoisies)}
                className="w-full py-4 rounded-2xl bg-pink-500 text-white font-black text-base flex items-center justify-center gap-2 active:bg-pink-600">
                <ShoppingCart className="w-5 h-5" />
                {produitDetail.type === "numerique"
                  ? `Commander — ${formatPrix(produitDetail.prix_promo || produitDetail.prix, boutique?.devise)}`
              ...
              {produitDetail.paiement_lien && (
                <a href={produitDetail.paiement_lien} target="_blank" rel="noopener noreferrer"
                  className="w-full py-3 rounded-2xl border-2 border-pink-500 text-pink-600 font-bold text-sm flex items-center justify-center gap-2">
                  <Link className="w-4 h-4" /> Payer directement en ligne
                </a>
              )}
            </div>
          </div>
